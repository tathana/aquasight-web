...
export async function POST(request:Request){
 ...
 try{const raw=await request.text();if(raw.length>12000)return Response.json({error:'Request too large'},{status:413});const result=await chat(JSON.parse(raw),createServices(env));return Response.json(result,{headers:{'Cache-Control':'no-store'}});}
