...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (config.N8N_WEBHOOK_URL) {
      ...
      try {
        const upstream = await fetch(config.N8N_WEBHOOK_URL, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: raw, signal: AbortSignal.timeout(55000) });
        if (!upstream.ok) throw new Error('n8n unavailable');
        const result = await upstream.json() as { text?: string };
        if (typeof result.text !== 'string') throw new Error('Invalid n8n response');
        return Response.json(result, { headers: { 'Cache-Control': 'no-store' } });
      } catch {
        return Response.json({ text: 'ยังเชื่อมต่อ n8n ไม่ได้ครับ กรุณาตรวจว่าระบบเปิดอยู่แล้วลองอีกครั้ง', context: body.context || {}, error: true, actions: [{ label: 'ลองอีกครั้ง', value: body.message }] });
      }
    }
    const result = await chat(body, createServices(config));
    return Response.json(result, { headers: { 'Cache-Control': 'no-store' } });
  } catch (e) {
